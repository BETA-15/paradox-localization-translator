...
- [x] 有効キー100件以上の下限を有効共通200件へ変更
- [x] 双方向40%と有効キー100件未満20%を維持
- [x] 複数Mod総合和訳の各寄与元を有効共通200件へ変更
- [x] 翻訳状況キャッシュ世代13と関係判定アルゴリズム世代2を追加
- [x] 199件FAIL / 200件PASSの境界値テスト
- [x] ERCF ← DITNが有効0・関連なしで、旧キャッシュも復元されないことを確認
- [x] 現在の88 Modで実データ再判定
- [x] pytest 26件 PASS
- [ ] GUI実画面で再調査後の関連付け表示を確認
- [ ] GitHub Actions 3OSビルドを確認

# v0.11.60 追加確認

- [x] `VERSION` / `APP_VERSION` / `pyproject.toml` を 0.11.60 に統一
- [x] 未変更バニラ持ち込みキーをMod間関係証拠から除外
- [x] 候補原文があるキーの原文一致・不一致を判定
- [x] 有効共通50件＋双方向40%ゲートを追加
- [x] 100有効キー未満の20%ゲートを維持
- [x] 本体70/80%・非本体20%・原文同梱20/80%の候補役割判定を追加
- [x] 複数Mod総合和訳の和集合70%ゲートを追加
- [x] 構成点だけでは自動関連付けしないことを確認
- [x] 判定ログへ生共通・バニラ除外・有効共通・原文照合・各ゲートを追加
- [x] ERCF ← DITN が生921 / バニラ921 / 有効0 / 関連なし
- [x] 現在の88 Modで新方式を実データ再判定
- [x] pytest 23件 PASS
- [ ] GUI実画面で判定ログ表示・書き出しを確認
- [ ] GitHub Actions 3OSビルドを確認

# v0.11.59 追加確認

- [x] `VERSION` / `APP_VERSION` / `pyproject.toml` を 0.11.59 に統一
- [x] 通常翻訳・中国語基準翻訳のキュー項目へ安定IDを付与
- [x] 翻訳中・差分準備中・上書き中の競合操作をグレーアウト
- [x] 通常キューと中国語キューの相互転送は、どちらかのキューが処理中ならグレーアウト
- [x] 保存場所変更は翻訳・診断・復元・上書き等の実行中にグレーアウト
- [x] 終了時にworker停止完了を待ってから正常終了を記録
- [x] 強制終了では正常終了マーカーを書かない
- [x] GUIイベント処理を最大200件・約12msで分割
- [x] 重要JSONの破損退避とエラーログ記録を追加
- [x] Python構文チェック PASS
- [x] pytest 13件 PASS
- [ ] GUI実画面で処理中のグレーアウトと復帰を確認
- [ ] GUI実画面で通常終了・10秒待機・強制終了を確認
- [ ] GitHub Actions 3OSビルドを確認

# v0.11.58 追加確認

- [x] `VERSION` / `APP_VERSION` を 0.11.58 に統一
- [x] 起動時に大きな `mod_status_cache` を同期JSON解析しない
- [x] 翻訳状況キャッシュ復元をバックグラウンド化
- [x] 総合診断キャッシュ復元をバックグラウンド化
- [x] バックアップ一覧の走査・翻訳状況からの復元先解決をバックグラウンド化
- [x] バックアップ一覧の重複更新要求をcoalesce
- [x] バックアップ復元のコピー処理をバックグラウンド化
- [x] 日本語化Mod判定ログの生成・書き出しをバックグラウンド化
- [x] QA解析をバックグラウンド化
- [x] 差分解析をバックグラウンド化し、一覧描画をチャンク化
- [x] 通常翻訳 / 中国語基準翻訳の差分準備（原文走査・欠損判定）をバックグラウンド化
- [x] 単体/一括Mod上書きをバックグラウンド化
- [x] 設定タブのデータ保存場所コピーをバックグラウンド化
- [x] 翻訳状況ライブ検索をデバウンス
- [x] 通常翻訳 / 中国語基準翻訳 / 翻訳検索 / 用語取り込み / 未翻訳監視 / 総合診断 / モデル接続の既存worker経路を確認
- [x] バックグラウンドスレッドからTk Widgetを直接更新しない
...
- [x] 100キー以上の日本語化Mod関係ゲートを50キー一致へ変更
- [x] 日本語localization専用候補を60点へ大幅加点
- [x] 部分翻訳Mod用の候補側一致例外ゲートを追加
- [x] バックアップ復元タブを追加
- [x] 上書きバックアップへ世代番号・対象Mod・原文状態・復元先を記録
- [x] 復元前退避を追加
- [x] 旧総合診断 / 旧上書きバックアップを一覧へ取り込み
...
- [x] 診断結果に「現在位置 / 修復予定 / 採用訳」を追加
- [x] 個別競合に「本体Modの訳 / 日本語化Modの訳 / 自動整理」の3モードを追加
- [x] 一括競合にも3モードを追加
- [x] 一括競合指定が統合先を直接変更しないことを確認
- [x] 手動で日本語化Modに分類された関係を自動整理で最優先
- [x] 選択行の詳細に手動例外の適用を表示
- [x] `python -m py_compile app/main.py app/translator_core.py` PASS
- [x] Xvfb GUI smoke test PASS
- [x] 合成テストで手動日本語化Mod分類が有効翻訳数より優先されることを確認

---

# v0.11.48 追加確認

- [x] `VERSION` / `APP_VERSION` を 0.11.48 に統一
- [x] 本体のみ / 日本語化Modのみの有効キーを統合対象として計数
- [x] 完全重複は手動競合にせず自動整理
- [x] 異なる訳文だけ手動優先指定を要求
- [x] 自動統合先は有効日本語キー数が多い側（同数は日本語化Mod側）
- [x] 統合対象の両 localization を修復前にバックアップ
- [x] 統合対象 / 訳文競合 / 未指定件数をUI表示
- [x] 競合0件では競合指定ボタンを無効化
- [x] 英語 / 簡体字中国語を変更しない

# v0.11.47 追加確認

- [x] `VERSION` / `APP_VERSION` を 0.11.47 に統一
- [x] 翻訳状況タブに「判定ログを表示」「判定ログを書き出す」を追加
- [x] 100キー以上50% / 100キー未満20%の適用ゲートをログ表示
- [x] 日本語一致率、候補側一致率、構成点、他言語関連度・減点、自動スコア、自動判定を表示
- [x] 手動の日本語化Mod / 通常Mod / 対応元Mod指定と最終判定を表示
- [x] 候補止まり・除外候補も判定ログへ出力
- [x] 判定ログを `ログ/translation_mod_judgement_*.txt` へUTF-8保存
- [x] `python -m py_compile app/main.py app/translator_core.py` PASS
